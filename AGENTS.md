...
- `empty_table_sql` renders `TRUNCATE TABLE`, overriding the ANSI `DELETE FROM` base: ClickHouse's `DELETE FROM` is a lightweight mutation whose grammar requires a `WHERE`, and a `DELETE ... WHERE 1` would only schedule an asynchronous mutation, leaving a truncate-insert's reset racing the append that follows it.
